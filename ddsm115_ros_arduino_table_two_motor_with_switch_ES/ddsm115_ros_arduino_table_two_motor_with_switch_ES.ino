...
// Emergency switch assign pin #14 and led pin #8
...
// ROS setup physical data 
uint16_t w_r=0, w_l=0;
//wheel_rad is the wheel radius ,wheel_sep is
double wheel_rad = 0.100, wheel_sep = 0.285;
...
void loop() {
    ...
    sendbuf[0] = (unsigned char)0x01;             // Motor ID=1 left
    sendbuf[1] = (unsigned char)0x64;             // Rotate motor command
    sendbuf[2] = (unsigned char)(w_l >> 8);       // Velocity high 8 bits
    sendbuf[3] = (unsigned char)(w_l & 0x00ff);   // Velocity low 8 bits
    sendbuf[4] = (unsigned char)0x00;             // Null data
    sendbuf[5] = (unsigned char)0x00;             // Null data
    sendbuf[6] = (unsigned char)0x02;             // Acceleration time
    sendbuf[7] = (unsigned char)Brake_P;          // Brake
    sendbuf[8] = (unsigned char)0x00;             // Null data
    ...
    sendbuf[0] = (unsigned char)0x02;            // Motor ID=2 right
    sendbuf[1] = (unsigned char)0x64;            // Rotate motor command
    sendbuf[2] = (unsigned char)(w_r >> 8);      // Velocity high 8 bits
    sendbuf[3] = (unsigned char)(w_r & 0x00ff);  // Velocity low 8 bits
    sendbuf[4] = (unsigned char)0x00;            // Null data
    sendbuf[5] = (unsigned char)0x00;            // Null data
    sendbuf[6] = (unsigned char)0x02;            // Acceleration time
    sendbuf[7] = (unsigned char)Brake_P;         // Brake
    ...
    // handling emergency switch and activation brake
    if(digitalRead(stop_button_pin1) == LOW){
    Brake_P = 0xFF;                 // activation brake
    digitalWrite(stop_lamp, HIGH);  //stop lamp on
    delay(10);  // Wait delay
    } else {
    Brake_P = 0x00;
